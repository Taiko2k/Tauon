...
class Gnome:
	...
	def start_indicator(self) -> None:
		...
		if self.loaded_indicator == "AyatanaAppIndicatorGlib":
			self.indicator = AyatanaAppIndicatorGlib.Indicator.new("Tauon", self.tauon.get_tray_icon("tray-indicator-default"), AyatanaAppIndicatorGlib.IndicatorCategory.APPLICATION_STATUS)
			...
			try:
				...
				class MPRIS(dbus.service.Object):
					...
					def update_progress(self) -> None:
						if (
							pctl.repeat_mode
							and not pctl.album_repeat_mode
							and pctl.playing_object() is not None
							and self.last_track_index == pctl.playing_object().index
							and pctl.playing_time < self.last_playing_time
							):
